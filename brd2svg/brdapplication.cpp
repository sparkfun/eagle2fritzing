...
void BrdApplication::genText(QDomElement & element, const QString & text, QString & svg, QDomElement & paramsRoot, const QString & textColor) 
{
    ...
    // Vector font = 0 & proportional font = 1
    int font = element.attribute("font", "").toInt(&ok);
    if (!ok) {
        qDebug() << "font not provided for" << text;
    }

    int align = element.attribute("align", "").toInt(&ok);
    if (!ok) {
        qDebug() << "alignment not provided for" << text;
        return;
    }
    ...
    bool anchorAtStart;
    QString horiz, vert;
    MiscUtils::calcTextAngle(align, angle, mirror, spin, size, x, y, anchorAtStart, horiz, vert);
    ...
    QString escapedString = TextUtils::escapeAnd(text);

    svg += QString("<text font-family='OCRA' stroke='%1' stroke-width='%2' fill='%3' font-size='%4' x='%5' y='%6' text-anchor='%7'style='dominant-baseline:%8;'%9>%10</text>\n")
            .arg(textColor)					// %1
            .arg(width)  					// %2 SW(width)
            .arg(textColor)					// %3
            .arg(size)						// %4
            .arg(x - m_trueBounds.left())	// %5
            .arg(flipy(y))					// %6
            .arg(horiz)						// %7
            .arg(vert)						// %8
            .arg(TextUtils::convertOverline(escapedString))	// %9
            .arg(escapedString)				// %10
            ;
    if (angle != 0) {
        svg += "</g></g>\n";
    }
